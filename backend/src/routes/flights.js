...
router.patch('/:id',
  ...
  async (req, res, next) => {
    try {
      ...
      console.log("The id is"+id +"req body"+req.body)
...
//delete Flight with cascade baggage deletion
router.delete('/:id',
  authRequired(['admin']), // Only admin can delete flights
  async (req, res, next) => {
    try {
      ...
      // Delete all baggage associated with this flight
      const deletedBaggage = await Baggage.deleteMany({ flightId: flight._id });
      ...
      await producer.send({
        ...
        messages: [{
          ...
          value: JSON.stringify({
            ...
            flightId: deletedFlight._id,
            flightNo: deletedFlight.flightNo,
            deletedBaggageCount: deletedBaggage.deletedCount,
            timestamp: new Date().toISOString()
          })
        }]
      });
      ...
      res.json({ 
        message: 'Flight and associated baggage deleted successfully',
        flightNo: deletedFlight.flightNo,
        deletedBaggageCount: deletedBaggage.deletedCount,
        deletedAt: new Date().toISOString()
      });
